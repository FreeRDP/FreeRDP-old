...
void credssp_ntlm_rc4k(uint8* key, int length, uint8* plaintext, uint8* ciphertext)
{
	CryptoRc4 rc4;
...
{
	uint8 generated_client_random[8];

	if (client_challenge == NULL)
	{
		/* Generate an 8-byte client random */
		credssp_ntlm_nonce(generated_client_random, 8);
		client_challenge = (uint8*) generated_client_random;
	}

	credssp_lm_v2_response_static(password, username, domain, client_challenge, server_challenge, lm_challenge_response);
}
...
void credssp_lm_v2_response_static(char* password, char* username, DATA_BLOB *domain, uint8* client_challenge, uint8* server_challenge, DATA_BLOB *lm_challenge_response)
{
	...
	memcpy(value, server_challenge, 8);
	memcpy(&value[8], client_challenge, 8);

	data_blob_alloc(lm_challenge_response, 24);
	response = lm_challenge_response->data;
	...
	memcpy(&response[16], client_challenge, 8);
}

void credssp_ntlm_current_time(uint8* timestamp)
{
	uint64 time64;
...
void credssp_ntlm_client_challenge(uint8* client_challenge)
{
	/* ClientChallenge used in NTLMv2 response */
	credssp_ntlm_nonce(client_challenge, 8);
}

void credssp_ntlm_client_random(uint8* client_challenge)
{
	/* ClientChallenge used in LMv2 response */
	credssp_ntlm_nonce(client_challenge, 8);
}

void credssp_ntlm_key_exchange_key(uint8* session_base_key, uint8* lm_challenge_response, uint8* server_challenge, uint8* key_exchange_key)
{
	/* In NTLMv2, KeyExchangeKey is the 128-bit SessionBaseKey */
	memcpy(key_exchange_key, session_base_key, 16);
}

void credssp_ntlm_exported_session_key(uint8* exported_session_key)
{
	credssp_ntlm_nonce(exported_session_key, 16);
}

void credssp_ntlm_random_session_key(uint8* random_session_key)
{
	credssp_ntlm_nonce(random_session_key, 16);
}
...
{
	/* In NTLMv2, EncrypteRandomSessionKey is the ExportedSessionKey RC4-encrypted with the KeyExchangeKey */
	credssp_ntlm_rc4k(key_exchange_key, 16, random_session_key, encrypted_random_session_key);
}
...
{
	credssp_ntlm_signing_key(exported_session_key, (uint8*) client_sign_magic, sizeof(client_sign_magic), signing_key);
}

void credssp_ntlm_server_signing_key(uint8* exported_session_key, uint8* signing_key)
{
	credssp_ntlm_signing_key(exported_session_key, (uint8*) server_sign_magic, sizeof(server_sign_magic), signing_key);
}

void credssp_ntlm_sealing_key(uint8* exported_session_key, uint8* seal_magic, int seal_magic_length, uint8* sealing_key)
{
	int length;
	uint8* value;
	CryptoMd5 md5;
...
{
	credssp_ntlm_sealing_key(exported_session_key, (uint8*) client_seal_magic, sizeof(client_seal_magic), sealing_key);
}

void credssp_ntlm_server_sealing_key(uint8* exported_session_key, uint8* sealing_key)
{
	credssp_ntlm_sealing_key(exported_session_key, (uint8*) server_seal_magic, sizeof(server_seal_magic), sealing_key);
}
...
void credssp_ntlm_make_signature(uint8* msg, int msg_len, uint8* signing_key, uint8* sealing_key, uint32 seq_num, CryptoRc4 *rc4, uint8* signature)
{
	...
	data_blob_alloc(&seq_num_msg, msg_len + 4);
	blob = (char*) seq_num_msg.data;
	memcpy(blob, &seq_num, 4);
	memcpy(&blob[4], msg, msg_len);

	/* Compute the HMAC-MD5 hash of the resulting value using the client signing key */
	HMAC(EVP_md5(), (void*) signing_key, 16, seq_num_msg.data, seq_num_msg.length, (void*) digest, NULL);
...
void credssp_ntlm_encrypt_message_with_signature(uint8* msg, int msg_len, uint8* signing_key, uint8* sealing_key, uint32 seq_num, CryptoRc4 *rc4, uint8* encrypted_message, uint8* signature)
{
	...
	crypto_rc4(*rc4, 8, &signature[4], checksum);
...
void credssp_ntlm_v2_response(char* password, char* username, DATA_BLOB *domain, uint8* client_challenge, uint8* server_challenge, uint8* client_random, DATA_BLOB *target_info, uint8* session_base_key, uint8* timestamp, DATA_BLOB *nt_challenge_response, DATA_BLOB *lm_challenge_response)
{
	...
	if (timestamp == NULL)
	{
		...
		timestamp = (uint8*) generated_timestamp;
	}
	...
	{
		credssp_ntlm_nonce(generated_client_random, 8);
		client_random = (uint8*) generated_client_random;
	}

	credssp_ntlm_v2_response_static(password, username, domain, client_challenge, server_challenge, client_random, target_info, session_base_key, timestamp, nt_challenge_response, lm_challenge_response);
}
...
void credssp_ntlm_v2_response_static(char* password, char* username, DATA_BLOB *domain, uint8* client_challenge, uint8* server_challenge, uint8* client_random, DATA_BLOB *target_info, uint8* session_base_key, uint8* timestamp, DATA_BLOB *nt_challenge_response, DATA_BLOB *lm_challenge_response)
{
	...
	data_blob_alloc(&ntlm_v2_temp, target_info->length + 28);
	memset(ntlm_v2_temp.data, '\0', ntlm_v2_temp.length);
	blob = (char*) ntlm_v2_temp.data;

	/* Compute the NTLMv2 hash */
	credssp_ntlm_v2_hash(password, username, domain, (char*) ntlm_v2_hash);
	...
	memcpy(&blob[16], client_challenge, 8); /* ClientChallenge (8 bytes) */
	/* Reserved3 (4 bytes) */
	...
	/* LmChallengeResponse */
	credssp_lm_v2_response(password, username, domain,
		client_random, server_challenge, lm_challenge_response);
...
void ntlm_recv_challenge_message(rdpSec * sec, STREAM s)
{
	...
	/* Generate RandomSessionKey */
	credssp_ntlm_random_session_key(sec->nla->random_session_key);

	/* Generate ExportedSessionKey */
	credssp_ntlm_exported_session_key(sec->nla->exported_session_key);

	/* Generate Client Challenge */
	credssp_ntlm_client_challenge((uint8*) sec->nla->client_challenge);

	/* Generate Client and Server Signing Keys */
	credssp_ntlm_client_signing_key(sec->nla->random_session_key, (uint8*) sec->nla->client_signing_key);

	/* Generate Client and Server Sealing Keys */
	credssp_ntlm_client_sealing_key(sec->nla->random_session_key, (uint8*) sec->nla->client_sealing_key);
...
void ntlm_send_authenticate_message(rdpSec * sec)
{
	...
	credssp_ntlm_v2_response(settings->password, settings->username,
		...
		NULL, &sec->nla->target_info, sec->nla->session_base_key, NULL,
	...
	credssp_ntlm_key_exchange_key(sec->nla->session_base_key, sec->nla->lm_challenge_response.data,
		sec->nla->server_challenge, sec->nla->key_exchange_key);

	EncryptedRandomSessionKeyLen = 16;
	EncryptedRandomSessionKeyBuffer = (uint8*) xmalloc(EncryptedRandomSessionKeyLen);

	credssp_ntlm_encrypt_random_session_key(sec->nla->random_session_key,
		sec->nla->key_exchange_key, EncryptedRandomSessionKeyBuffer);
