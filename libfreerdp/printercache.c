...
printercache_process(STREAM s)
{
	...
	switch (type)
	{
		case 4:	/* rename item */
			...
			rdp_in_unistr(g_rdp, s, printer, sizeof(printer), printer_length);
			rdp_in_unistr(g_rdp, s, driver, sizeof(driver), driver_length);
		...
		case 3:	/* delete item */
			...
			in_uint8s(s, 0x3);	/* padding */
			rdp_in_unistr(g_rdp, s, printer, sizeof(printer), printer_unicode_length);
		...
		default:
			...
			ui_unimpl(NULL, "RDPDR Printer Cache Packet Type: %d\n", type);
			break;
	}
}
