...
rdpsnd_init_packet(uint16 type, uint16 size)
{
	...
	s = channel_init(g_rdp->sec->mcs->chan, rdpsnd_channel, size + 4);
...
{
	channel_send(g_rdp->sec->mcs->chan, s, rdpsnd_channel);
}
...
rdpsnd_send_completion(uint16 tick, uint8 packet_index)
{
	...
	DEBUG_SOUND("-> RDPSND_COMPLETION(tick: %u, index: %u)\n",
...
rdpsnd_flush_record(void)
{
	...
	while (record_buffer_size)
	{
		...
		DEBUG_SOUND("-> RDPSND_REC_DATA(length: %u)\n", (unsigned) chunk_size);
	}
...
rdpsnd_process_negotiate(STREAM in)
{
	...
	DEBUG_SOUND("RDPSND_NEGOTIATE(formats: %d, pad: 0x%02x, version: %x)\n",
	...
	DEBUG_SOUND("-> RDPSND_NEGOTIATE(formats: %d)\n", (int) format_count);
...
rdpsnd_process_ping(STREAM in)
{
	...
	DEBUG_SOUND("RDPSND_PING(tick: 0x%04x)\n", (unsigned) tick);
...
rdpsnd_process_rec_negotiate(STREAM in)
{
	...
	DEBUG_SOUND("RDPSND_REC_NEGOTIATE(formats: %d, version: %x)\n",
	...
	DEBUG_SOUND("-> RDPSND_REC_NEGOTIATE(formats: %d)\n", (int) rec_format_count);
...
rdpsnd_process_packet(uint8 opcode, STREAM s)
{
	...
	switch (opcode)
	{
		...
		case RDPSND_REC_START:
			in_uint16_le(s, format);
			DEBUG_SOUND("RDPSND_REC_START(format: %u)\n", (unsigned) format);
		...
		case RDPSND_REC_SET_VOLUME:
			in_uint16_le(s, vol_left);
			in_uint16_le(s, vol_right);
			DEBUG_SOUND("RDPSND_REC_VOLUME(left: 0x%04x (%u %%), right: 0x%04x (%u %%))\n", (unsigned) vol_left, (unsigned) vol_left / 655, (unsigned) vol_right, (unsigned) vol_right / 655);
			if (rec_device_open)
	...
	while (!s_check_end(s))
	{
		...
		else
		{
			...
			if (packet_opcode == RDPSND_WRITE)
			{
				...
				{
					DEBUG_SOUND("Eating 4 bytes of %d bytes...\n",
						     len);
					in_uint8s(s, 4);
					len -= 4;
				}
...
rdpsnd_init(char *optarg)
{
	...
	rdpsnd_channel =
		channel_register(g_rdp->sec->mcs->chan, "rdpsnd", CHANNEL_OPTION_INITIALIZED | CHANNEL_OPTION_ENCRYPT_RDP,
				 rdpsnd_process);

	rdpsnddbg_channel =
		channel_register(g_rdp->sec->mcs->chan, "snddbg", CHANNEL_OPTION_INITIALIZED | CHANNEL_OPTION_ENCRYPT_RDP,
				 rdpsnddbg_process);
