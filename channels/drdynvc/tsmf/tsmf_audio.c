...
static ITSMFAudioDevice *
tsmf_load_audio_device_by_name(const char * name, const char * device)
{
	...
	han = DLOPEN(path);
	LLOGLN(0, ("tsmf_load_audio_device_by_name: %s", path));
	if (han == NULL)
	{
		LLOGLN(0, ("tsmf_load_audio_device_by_name: failed to load %s", path));
		return NULL;
	}
	entry = (TSMF_AUDIO_DEVICE_ENTRY) DLSYM(han, TSMF_AUDIO_DEVICE_EXPORT_FUNC_NAME);
	if (entry == NULL)
	{
		DLCLOSE(han);
		LLOGLN(0, ("tsmf_load_audio_device_by_name: failed to find export function in %s", path));
		return NULL;
	}
	audio = entry();
	if (audio == NULL)
	{
		DLCLOSE(han);
		LLOGLN(0, ("tsmf_load_audio_device_by_name: failed to call export function in %s", path));
		return NULL;
	}
	if (audio->Open(audio, device) != 0)
	{
		audio->Free(audio);
		audio = NULL;
	}
