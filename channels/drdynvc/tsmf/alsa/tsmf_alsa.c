...
tsmf_alsa_open(ITSMFAudioDevice * audio, const char * device)
{
	...
	if (error < 0)
	{
		LLOGLN(0, ("tsmf_alsa_open: failed to open device %s", alsa->device));
		return 1;
	}
...
tsmf_alsa_set_format(ITSMFAudioDevice * audio, uint32 sample_rate, uint32 channels, uint32 bits_per_sample)
{
	...
	snd_pcm_sw_params_set_start_threshold(alsa->out_handle, sw_params,
		frames / 2 < alsa->actual_rate / 2 ? frames / 2 : alsa->actual_rate / 2);
...
tsmf_alsa_play(ITSMFAudioDevice * audio, uint8 * data, uint32 data_size)
{
	...
	LLOGLN(0, ("tsmf_alsa_play: data_size %d", data_size));
	if (alsa->out_handle)
	{
		...
		while (pindex < end)
		{
			...
			else if (error < 0)
			{
				...
				alsa->out_handle = 0;
				tsmf_alsa_open(audio, NULL);
				break;
			}
			LLOGLN(0, ("tsmf_alsa_play: %d frames played.", error));
			if (error == 0)
				break;
