...
handle_child_line(const char *line, void *data)
{
	...
	{
		ui_warning(NULL, "lspci: Unrecoqnized line '%s'\n", line);
	}
	return True;
}
...
lspci_process_line(const char *line, void *data)
{
	...
	{
		ui_error(NULL, "lspci protocol error: Invalid line '%s'\n", line);
	}
	return True;
}
...
{
	lspci_channel =
		channel_register(g_rdp->sec->mcs->chan, "lspci", CHANNEL_OPTION_INITIALIZED | CHANNEL_OPTION_ENCRYPT_RDP,
				 lspci_process);
...
lspci_send(const char *output)
{
	...
	len = strlen(output);
	s = channel_init(g_rdp->sec->mcs->chan, lspci_channel, len);
	...
	channel_send(g_rdp->sec->mcs->chan, s, lspci_channel);
}
