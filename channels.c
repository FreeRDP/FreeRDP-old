...
channel_register(rdpChannels * chan, char *name, uint32 flags, void (*callback) (STREAM))
{
	...
	if (chan->mcs->sec->rdp->settings->rdp_version < 5)
		return NULL;

	if (chan->num_channels >= MAX_CHANNELS)
	{
		ui_error(chan->mcs->sec->rdp->inst, "Channel table full, increase MAX_CHANNELS\n");
		return NULL;
	}
...
channel_init(rdpChannels * chan, VCHANNEL * channel, uint32 length)
{
	...
	s = sec_init(chan->mcs->sec, chan->mcs->sec->rdp->settings->encryption ? SEC_ENCRYPT : 0,
		     length + 8);
